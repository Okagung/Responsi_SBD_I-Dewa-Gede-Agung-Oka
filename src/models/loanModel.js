  ...
  async getTopBorrowers() {
    const query = `
      WITH MemberLoanCounts AS (
          SELECT member_id, COUNT(*) as total_loans, MAX(loan_date) as last_loan_date
          FROM loans
          GROUP BY member_id
      ),
      RankedBooks AS (
          SELECT l.member_id, b.title, COUNT(*) as borrow_count,
      ...
      SELECT 
          ...
          mlc.last_loan_date, 
          rb.title as favorite_book
      FROM members m
      JOIN MemberLoanCounts mlc ON m.id = mlc.member_id
      LEFT JOIN RankedBooks rb ON m.id = rb.member_id AND rb.rn = 1
